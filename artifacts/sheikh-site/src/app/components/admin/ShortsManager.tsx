...
import { ShortFormDialog } from "./ShortFormDialog";

type Feedback = { type: "success" | "error"; message: string };
type Props = { onMutate?: () => void };

function publishBadgeClass(status: ShortClipItem["publishStatus"]) {
  if (status === "منشور") return "bg-green-100 text-green-800";
  if (status === "مخفي") return "bg-gray-200 text-gray-700";
  return "bg-amber-100 text-amber-800";
}

export function ShortsManager({ onMutate }: Props) {
  const [items, setItems] = useState<ShortClipItem[]>([]);
  ...
  const [editing, setEditing] = useState<ShortClipItem | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<ShortClipItem | null>(null);
  ...
  const loadShorts = useCallback(async () => {
    ...
    setError("");
    try {
      const result = await adminApi.listShorts({
        search: search.trim() || undefined,
        publishStatus: publishFilter || undefined,
      });
      setItems(result.items);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "تعذر تحميل المقاطع.");
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const handle = setTimeout(() => void loadShorts(), 300);
  ...
  function openEdit(item: ShortClipItem) {
    setEditing(item);
    setDialogOpen(true);
  }
  ...
  async function confirmDelete() {
    ...
    setDeleting(true);
    try {
      await adminApi.deleteShort(deleteTarget.id);
      showFeedback({ type: "success", message: "تم حذف المقطع بنجاح." });
      ...
      showFeedback({
        type: "error",
        message: err instanceof ApiError ? err.message : "تعذر حذف المقطع.",
      });
    } finally {
      setDeleting(false);
    }
            ...
            {loading ? (
              <tr>
                <td colSpan={5} className="px-5 py-12 text-center text-gray-500">
                  ...
                  <p className="mt-2">جارٍ تحميل المقاطع...</p>
                </td>
              </tr>
            ) : items.length > 0 ? (
              items.map((item) => (
              ...
              <tr>
                <td colSpan={5} className="px-5 py-12 text-center text-gray-500">
                  لا توجد مقاطع مطابقة. ابدأ بإضافة مقطع جديد.
                </td>
              </tr>
            )}
          </tbody>
