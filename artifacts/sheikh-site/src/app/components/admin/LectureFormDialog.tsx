...
type FormState = {
  title: string;
  url: string;
  lectureType: LectureItem["lectureType"];
  channel: string;
  videoId: string;
  duration: string;
  dateHijri: string;
  dateGregorian: string;
  trust: LectureItem["trust"];
  publishStatus: LectureItem["publishStatus"];
  displayOrder: string;
  categoryId: string;
...
function toFormState(item: LectureItem | null): FormState {
  return {
    title: item?.title ?? "",
    url: item?.url ?? "",
    lectureType: item?.lectureType ?? "محاضرة عامة",
...
export function LectureFormDialog({ item, categories, onClose, onSaved }: Props) {
  ...
  async function requestSuggestion(title: string, channel: string) {
    ...
    try {
      ...
      // Auto-apply only when confident and the field is still empty.
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      if (item) {
        await adminApi.updateLecture(item.id, payload);
      } else {
        await adminApi.createLecture(payload);
      }
      onSaved(isEdit ? "تم تحديث المحاضرة بنجاح." : "تمت إضافة المحاضرة بنجاح.");
    } catch (err) {
      if (err instanceof ApiError) {
        if (err.fields) setErrors(err.fields);
        setGeneralError(err.message);
      } else {
        setGeneralError("تعذر حفظ المحاضرة. حاول مرة أخرى.");
      }
      setSaving(false);
    }
  }
      ...
      <div className="w-full max-w-3xl rounded-sm border border-gray-200 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-200 p-5">
          <h2 className="font-serif text-2xl font-bold text-[var(--color-islamic-green-dark)]">
            {isEdit ? "تعديل محاضرة" : "إضافة محاضرة"}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              <label className={labelClass}>عنوان المحاضرة *</label>
              <input
                className={fieldClass}
                value={form.title}
                onChange={(e) => update("title", e.target.value)}
                placeholder="مثال: أهمية طلب العلم"
              />
              {errors.title && (
            ...
            <div className="md:col-span-2">
              <label className={labelClass}>رابط المحاضرة (YouTube) *</label>
              <input
                className={fieldClass}
                value={form.url}
            ...
            <div>
              <label className={labelClass}>نوع المحاضرة</label>
              <select
                className={fieldClass}
                value={form.lectureType}
                onChange={(e) =>
                  update("lectureType", e.target.value as FormState["lectureType"])
                }
              >
