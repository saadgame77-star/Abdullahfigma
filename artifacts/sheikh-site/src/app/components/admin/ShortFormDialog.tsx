...
type FormState = {
  ...
  trust: ShortClipItem["trust"];
  publishStatus: ShortClipItem["publishStatus"];
  displayOrder: string;
  categoryId: string;
...
function toFormState(item: ShortClipItem | null): FormState {
  return {
    title: item?.title ?? "",
    url: item?.url ?? "",
...
export function ShortFormDialog({ item, categories, onClose, onSaved }: Props) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      if (item) await adminApi.updateShort(item.id, payload);
      else await adminApi.createShort(payload);
      onSaved(isEdit ? "تم تحديث المقطع بنجاح." : "تمت إضافة المقطع بنجاح.");
    } catch (err) {
      if (err instanceof ApiError) {
        if (err.fields) setErrors(err.fields);
        setGeneralError(err.message);
      } else {
        setGeneralError("تعذر حفظ المقطع. حاول مرة أخرى.");
      }
      setSaving(false);
    }
  }
      ...
      <div className="w-full max-w-3xl rounded-sm border border-gray-200 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-200 p-5">
          <h2 className="font-serif text-2xl font-bold text-[var(--color-islamic-green-dark)]">
            {isEdit ? "تعديل مقطع قصير" : "إضافة مقطع قصير"}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              <label className={labelClass}>عنوان المقطع *</label>
              <input
                className={fieldClass}
                value={form.title}
                onChange={(e) => update("title", e.target.value)}
                placeholder="مثال: فائدة في الإخلاص"
              />
              {errors.title && (
            ...
            <div className="md:col-span-2">
              <label className={labelClass}>رابط المقطع (YouTube) *</label>
              <input
                className={fieldClass}
                value={form.url}
            ...
            <div>
              <label className={labelClass}>المدة بالثواني</label>
              <input
                type="number"
                min={0}
                className={fieldClass}
          ...
          <div className="mt-6 flex items-center justify-end gap-3 border-t border-gray-200 pt-5">
            ...
            >
              ...
              {isEdit ? "حفظ التعديلات" : "إضافة المقطع"}
            </button>
          </div>
        </form>
      </div>
